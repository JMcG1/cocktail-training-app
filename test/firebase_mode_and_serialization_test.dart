...
void main() {
  ...
  group('Firestore serializers', () {
    test('round trips persisted models', () {
      ...
      final draft = RecipeImportDraft(
        ...
        notes: '',
        ingredients: const [
          RecipeIngredient(ingredientName: 'Lyre\'s Italian Spritz', measureMl: 35),
        ],
      ...
      final recipeRoundTrip =
          FirestoreSerializers.recipeFromMap('recipe-1', FirestoreSerializers.recipeToMap(recipe));
      final draftRoundTrip =
          FirestoreSerializers.draftFromMap('draft-1', FirestoreSerializers.draftToMap(draft));
  ...
  group('Onboarding and controller wiring', () {
    ...
    test('user can only access their own venue data through configured venue scope', () async {
      final auth = _ConfigurableAuthRepository(
      ...
      await controller.signInManager(email: 'manager@venue.com', password: 'password');
    ...
    test('setup checklist state changes as data is added', () async {
      ...
      controller.saveIngredient(name: 'Vodka', bottleSizeMl: 700, bottleCost: 28);
